...
@section('title', 'Редактировать категорию - Админ-панель - EInvestor')
...
<div class="content">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" style="max-width: 800px; margin-top: 2rem;">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; gap: 1.5rem;">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          style="width: 100%; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 4px;">{{ old('description', $category->description) }}</textarea>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div>
                    ...
                    <select name="type" required style="width: 100%; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 4px;">
                        <option value="post" {{ old('type', $category->type) === 'post' ? 'selected' : '' }}>Статьи</option>
                        <option value="product" {{ old('type', $category->type) === 'product' ? 'selected' : '' }}>Товары</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="seo_description" rows="3" maxlength="320"
                          style="width: 100%; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 4px;">{{ old('seo_description', $category->seo_description) }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="seo_h1" value="{{ old('seo_h1', $category->seo_h1) }}" maxlength="255"
                       style="width: 100%; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 4px;">
            ...
            <div>
                ...
                <textarea name="seo_intro_text" rows="3"
                          style="width: 100%; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 4px;">{{ old('seo_intro_text', $category->seo_intro_text) }}</textarea>
